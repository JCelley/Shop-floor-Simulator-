// Same check as verify-realpipeline-o1160.js, for O1224 - using the ACTUAL engine.js code path
// app.js now runs (TRI_DEXEL_TOL_DEG=5, fuseTriDexel with obliqueSims/planeById), against ground
// truth. O1224's oblique plane (7) has zero real cutting moves in this fixture, so the zBot-scope
// fix shouldn't change anything here - this confirms that rather than assuming it.
'use strict';
...
fs.writeFileSync(path.join(__dirname, 'out-verify-realpipeline-o1224.json'), JSON.stringify({ pos: Array.from(pos), idx: Array.from(idx), box: stockBox }));
