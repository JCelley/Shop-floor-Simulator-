...
(async () => {
  ...
  const data = JSON.parse(fs.readFileSync(path.join(__dirname, 'out-worlddexel-o1160.json'), 'utf8'));
  ...
  for (const angle of [0, 1, 2]) {
    ...
    await page.screenshot({ path: path.join(__dirname, `out-worlddexel-o1160-${angle}.png`) });
  }
  await browser.close();
  console.log('done');
