...
v2.7.1 - 9/22/26 - Stopped writing the v2.6.7 "<base>.floorsim.json". The CIMCO scanning output
  (.setup + STOCK/PART/FIXTURE STL) added in v2.7.0 covers everything it did, plus real fixture/part/
  tool data it never had. Keeping both around actively caused a real problem: FloorSim's loader always
  prefers a plain "<base>.floorsim.json" over the CIMCO setup when both are selected, so an operator
  selecting every file in the posted folder at once silently got the old JSON's bare stock box instead
  of the real fixture, with no warning. UNTESTED IN
  FUSION until run for real.
v2.7.2 - 9/24/26 - Adds one extra "TOOLGEOM" line per tool to the .setup file, right after its TOOL
  line, so FloorSim can simulate undercut tools (lollipop, T-slot, dovetail) with their real shape.
  Autodesk's TOOL line never carries the neck size: its "SD" (labelled "shoulder diameter") is
  really just tool.diameter again. Fusion's post Tool object has no neck-diameter property either
  (checked against Autodesk's Tool class reference), so this writes every candidate that might hold
  it - the tool's shaft sections (tool.getShaft(), diameter/length per section) and a few tool
  parameters - and one real test post shows which one is right. Nothing else changes: CSV, NC,
  TOOL/HOLDER lines and STL files are untouched, and the extra line is wrapped in its own try/catch
  so it can never break the post. Older FloorSim builds ignore the line. UNTESTED IN FUSION.
*/

description = "Output CSV for ProShop v2.7.2 (+ CIMCO scanning)";
vendor = "APW";
...
// v2.7.2 - one "TOOLGEOM <no> KEY=value ..." line per tool for FloorSim (see the v2.7.2 note at the
// top). Every read is guarded separately: a property this post engine doesn't have just reads "NA"
// instead of stopping the line. SHAFT = the tool's shaft sections from the tip upward as
// diameter:length pairs separated by "/" (empty if Fusion stores none). P_* = tool parameters read
// through the same getSectionParameterForTool() Autodesk's own code above uses for tip angle.
function writeToolGeometry(tool) {
  try {
    var num = function (v) { return (typeof v == "number" && isFinite(v)) ? valueFormat.format(v) : "NA"; };
    var read = function (f) { try { return f(); } catch (e) { return undefined; } };
    var words = ["TOOLGEOM", tool.number, "TYPE=" + String(getToolName(tool)).replace(/ /g, "_"), unit == MM ? "US=UM" : "US=UI"];
    words.push("D=" + num(read(function () { return tool.diameter; })));
    words.push("CR=" + num(read(function () { return tool.cornerRadius; })));
    words.push("TA=" + num(read(function () { return tool.taperAngle; })));
    words.push("FL=" + num(read(function () { return tool.fluteLength; })));
    words.push("SL=" + num(read(function () { return tool.shoulderLength; })));
    words.push("SHD=" + num(read(function () { return tool.shaftDiameter; })));
    words.push("BL=" + num(read(function () { return tool.bodyLength; })));
    words.push("OAL=" + num(read(function () { return tool.overallLength; })));
    var shaft = read(function () { return tool.getShaft(); }), parts = [];
    if (shaft && read(function () { return shaft.hasSections(); })) {
      var n = read(function () { return shaft.getNumberOfSections(); }) || 0;
      for (var i = 0; i < n; ++i) {
        parts.push(num(read(function () { return shaft.getDiameter(i); })) + ":" + num(read(function () { return shaft.getLength(i); })));
      }
    }
    words.push("SHAFT=" + (shaft ? (parts.length ? parts.join("/") : "EMPTY") : "NA"));
    var params = ["tool_shoulderDiameter", "tool_neckDiameter", "tool_shaftDiameter", "tool_shoulderLength", "tool_fluteLength", "tool_bodyLength", "tool_taperAngle", "tool_tipDiameter"];
    for (var p = 0; p < params.length; ++p) {
      var v = read(function () { return getSectionParameterForTool(tool, "operation:" + params[p], "NA"); });
      words.push("P_" + params[p].replace("tool_", "") + "=" + (typeof v == "number" ? valueFormat.format(v) : String(v == undefined ? "NA" : v).replace(/ /g, "_")));
    }
    cimcoFile.writeln(words.join(" "));
  } catch (e) {
    try { cimcoFile.writeln("TOOLGEOM " + tool.number + " ERROR=" + String(e).replace(/\s+/g, "_").substring(0, 120)); } catch (e2) { /* never break the post */ }
  }
}
...
// onClose() inside its own try/catch, so a problem here can never affect the CSV.
...
for (var i = 0; i < n; ++i) {
  ...
  // if this fails for any reason, the CSV above is already written and unaffected.
  if (cimcoSkipped) {
    writeln(localize("CIMCO scanning files SKIPPED (CSV above is unaffected): " + cimcoSkipReason));
  } else {
    try {
      writeCimcoScanningFiles();
    } catch (eCimco) {
      writeln(localize("CIMCO scanning files FAILED (CSV above is unaffected): " + eCimco));
    }
  }
}
