...
const logo = fs.readFileSync(path.join(root, 'src', 'apw-logo.webp')).toString('base64');   // APW logo, from americanprecisionworks.com
const html = rd('index.template.html').replace('/*LOGO*/', () => logo).replace('/*CSS*/', () => rd('style.css')).replace('/*ENGINE*/', () => rd('engine.js')).replace('/*APP*/', () => rd('app.js'));
